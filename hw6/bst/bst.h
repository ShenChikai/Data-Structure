...
{
	if(mRoot == NULL){	// if empty
  		mRoot = new Node<Key, Value>(keyValuePair.first, keyValuePair.second, NULL);
 	} else {
  		Node<Key, Value>* temp = mRoot;	// iterator
  		while(temp != NULL){
	   		if (keyValuePair.first < temp->getItem().first){	// insert to left
	    		if(temp->getLeft() == NULL){	// found place

	     			Node<Key,Value> * node = new Node<Key, Value>(keyValuePair.first, keyValuePair.second, temp);
	                temp->setLeft(node);
	    		 	break; 
	    		} else{
	    			temp = temp->getLeft();
	   		...
	   		} else if(keyValuePair.first > temp->getItem().first){	// insert to right
	   	 		if(temp->getRight() == NULL){	// found place
	   		...
	   		} else { 	// SAME KEY FOUND, UPDATE Value
...
Node<Key, Value>* BinarySearchTree<Key, Value>::internalFind(const Key& key) const
{
	...
	return internalHelper(key, mRoot);
}

template<typename Key, typename Value>
Node<Key, Value>* internalHelper(const Key& key, Node<Key, Value>* n) {

    if (n == NULL || n->getKey() == key) return n; 
    
    if (n->getKey() < key) {
    	return internalHelper(key, n->getRight()); // Key is greater than root's key 
  	} else {
  		return internalHelper(key, n->getLeft());  // Key is smaller than root's key 
  	}
...
*/
			// comment to use the pretty print
